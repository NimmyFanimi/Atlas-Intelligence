...
3. **Morning Brief**: generated daily summary combining overnight moves, main driver, today's key risk events — **BACKEND COMPLETE (Week 3), frontend + cron route not yet built**
4. **Economic Calendar**: upcoming data releases with importance, previous/forecast/actual, plain-English explanation of what each metric measures and why traders watch it — **COMPLETE (Week 3)**
...
### Economic Calendar data source (Week 3): FRED, with two real bugs found and fixed

Before settling on FRED, three paid/unverified alternatives were tested and ruled out: **Trading Economics** (no confirmed self-serve free tier found, every pricing path led to a sales/quote flow), **Finnhub's economic calendar endpoint** (confirmed paid-only via a live 403 test against a real key, `{"error":"You don't have access to this resource."}`), and **FXStreet** (sales-gated, no public pricing). This established the project's £0-budget calendar options were genuinely limited to FRED, an AI-estimated consensus layer (see Known Gaps), or an unreliable third-party scraper/wrapper (explicitly rejected as too fragile for an unattended project running through application season).

**FRED's release-dates endpoint has a real, non-obvious gotcha, found via live testing, not documentation**: there are two structurally different endpoints. `fred/releases/dates` (plural) returns FRED's entire catalog of ~5600 dated entries across every release, and **silently ignores the `release_id` query parameter entirely** (verified: a request with `release_id=10` for CPI returned entries for release_id 101, 441, 86, etc., completely unfiltered). The correct endpoint is `fred/release/dates` (singular), which genuinely filters by `release_id` as documented.

A second bug surfaced even after switching to the correct endpoint: using `sort_order=asc` with a `limit=1000` worked for low-frequency releases like CPI (947 total historical entries, comfortably fits) but failed silently for FOMC (`release_id=101`, which has **3748** total dated entries due to being tied to daily-updating rate-related series) — a 1000-entry ascending slice starting from 2000 never reached present-day dates. The fix: use `sort_order=desc&limit=20` (a small number of the most recent entries), filter to `date >= today`, then take the **last** entry in that filtered array (since the list stays descending, the last future-dated entry is the soonest one). This is documented as the exact implementation in `fetchNextReleaseDate` (`src/lib/data-sources/fred.ts`) and must not be "simplified" back to ascending+large-limit without re-verifying against a high-frequency release like FOMC.
...
**`calendar_events` table** (Week 3, cron-populated by `ingestUpcomingCalendarEvents()`):
- `id` (uuid, PK), `fred_release_id` (text), `series_id` (text, nullable)
- `event_name` (text, currently stores the `primaryName` from `TRACKED_RELEASES`, e.g. "US CPI" — the full/secondary name is frontend-display-only, looked up from `TRACKED_RELEASES` by `fred_release_id` at render time, deliberately NOT duplicated into the DB since it's static config already living in one place)
- `country` (text)
- `category` (text, CHECK constraint: `inflation`|`employment`|`rates`|`growth`|`manufacturing`)
- `importance` (text, CHECK constraint: `high`|`medium`|`low`, defaults `medium`)
- `release_date` (date), `release_time` (time, nullable), `period_covered` (text, nullable)
- `actual_value`, `previous_value`, `estimated_consensus`, `estimate_rationale` (all text, nullable) — deliberately left unset by the current ingestion pass, these are for a later "AI-estimated consensus" step (see Known Gaps) that hasn't been built yet
- `estimate_generated_at`, `estimate_model_used` (nullable) — same, for the future estimate step
- `status` (text, CHECK: `scheduled`|`released`|`revised`, defaults `scheduled`)
- Timestamps. Unique index on `(fred_release_id, release_date)` (the dedup/upsert key). Partial index on rows where `estimated_consensus IS NULL AND status = 'scheduled'` (for the future estimate-generation cron's query pattern). Public-read RLS.

Text was chosen over numeric for `actual_value`/`previous_value`/`estimated_consensus` deliberately: economic releases mix units wildly (percentages, index points, thousands of jobs), and forcing everything into one numeric column would lose unit context, at the cost of not being able to do numeric comparisons in SQL directly. Reasonable tradeoff for a display-focused module, not an analytics engine.

**`morning_briefs` table** (Week 3, cron-populated by `generateMorningBrief()`):
- `id` (uuid, PK), `brief_date` (date, `UNIQUE` — hard guarantee of one brief per day at the DB level, also the upsert conflict key)
- `content` (text, the Gemini-generated 200-300 word narrative)
- `movers_data` (jsonb — top-3-gainers/top-3-losers, each with symbol/name/change_pct; jsonb chosen here unlike `calendar_events`'s flat text columns because this is genuinely a structured multi-item list, matching how `news_articles.ai_analysis` already uses jsonb for structured AI output)
- `ai_model_used` (text), `generated_at`, `created_at`
- No `updated_at`: unlike `calendar_events` (which mutates as dates get confirmed/released), a brief for a given date is write-once, never revised after generation.
- Public-read RLS.
...
**Economic Calendar (Week 3, complete):**
- `src/lib/data-sources/fred.ts` — added `fetchNextReleaseDate(releaseId)`, see the Data Sources section above for the two real bugs found and fixed here (plural-vs-singular endpoint, ascending-vs-descending sort for high-frequency releases). Do not touch this function's core approach without re-reading that section first.
- `src/lib/data-sources/tracked-releases.ts` — `TRACKED_RELEASES`, the curated V1 FRED allowlist: CPI (`10`), NFP (`50`), GDP (`53`), PCE (`54`). FOMC removed from this list — FRED release 101 ("FOMC Press Release") was a daily-updating interest-rate series, not an FOMC meeting calendar; see fomc-calendar.ts for the replacement source. Each entry has `primaryName` (short/acronym, e.g. "US NFP") and `secondaryName` (full name, e.g. "Employment Situation"), plus `category`/`importance`/`country`. ISM/PMI deliberately excluded, FRED simply does not carry it in its release catalog (confirmed by searching the full ~330-release list), this is a source limitation not a budget one.
- `src/lib/data-sources/fomc-calendar.ts` — `fetchUpcomingFomcMeetings()`, fetches FOMC meeting schedule from the unofficial the-calendar.net mirror (NOT an official Fed or FRED source — see sourcing transparency note in the file). Returns only Day 2 (rate-decision) meetings. Used by calendar-ingestion.ts alongside FRED-based releases; failure here does not block CPI/NFP/GDP/PCE ingestion.
- `src/lib/calendar-ingestion.ts` — `ingestUpcomingCalendarEvents()`, loops `TRACKED_RELEASES` (FRED events) + calls `fetchUpcomingFomcMeetings()` (FOMC), upserts on `(fred_release_id, release_date)` with `data_source` column distinguishing FRED vs non-FRED rows, per-entry error isolation matching the style already established in `ingest-market-snapshots.ts`.
- `src/app/api/cron/calendar-ingest/route.ts` — cron route, auth matches `news-ingest`'s constant-time `CRON_SECRET` comparison exactly (including a length-guard before calling `timingSafeEqual`, which throws on mismatched-length buffers otherwise). **Live and registered on cron-job.org, running daily, failure email notifications on.**
- `scripts/check-release-dates.ts` — diagnostic script that hits FRED's `release/dates` endpoint per `TRACKED_RELEASES` entry (mirrors the exact production logic in `fetchNextReleaseDate`, not a simplified version) to confirm whether a date has been published yet (run: `npx tsx scripts/check-release-dates.ts`); helps distinguish "real ingestion bug" from "FRED hasn't published a date yet" during reporting disruptions/government shutdowns. Loads `.env.local` explicitly (`import { config } from 'dotenv'; config({ path: '.env.local' })`), not the `dotenv` package default of `.env`, this project's real env file is `.env.local` and the plain `import 'dotenv/config'` pattern silently fails to find it, a real bug caught this session.
- `src/app/calendar/page.tsx` — server component. Fetches `calendar_events`, builds a 7-day-forward window but **filters to only days with at least 1 event** (not just trailing empties, ALL empty days are dropped, this was a deliberate UX fix after the initial "trailing only" version still forced scrolling past a wall of empty days to reach genuinely useful content), computes `nextEvent` as the earliest row with `release_date >= today` (explicitly excludes stale past-dated rows), computes `tbdReleases` as any `TRACKED_RELEASES` entry with zero matching rows in `calendar_events`.
- `src/components/calendar/CalendarView.tsx` — client component, renders the v4 mockup design exactly: day-grouped cards, left accent bar = importance only (via `--color-accent` teal, **never** the reserved `--color-market-up`/`--color-market-down` sage/coral tokens), status pill = Confirmed (teal) or Date TBD (muted grey, hover tooltip), header "NEXT - `<event>` · `<date>`", "No confirmed releases this week" fallback when the filtered window is empty.
- `src/components/ui/Sidebar.tsx` — Economic Calendar nav item switched from `kind: 'coming-soon'` to a real active route pointing at `/calendar`. Also: fixed a white-box active-state bug in the shared `NavItem` component (a border/focus-ring only visible when navigating TO a route, not away from it, so it had only ever shown up on Calendar specifically since the other two active pages are never themselves the destination of a fresh click in normal use) by removing the border override and adding `outline-none`, this benefits all nav items.

**App-wide scrollbar fix (Week 3, not calendar-specific but found while testing Calendar):** a `.scrollbar-none` utility class already existed in `globals.css` but was never applied anywhere. Applied this session to 4 separate scroll containers across 3 pages: `AppShell.tsx`'s `<main>`, `NewsFeed.tsx`'s article modal (`fixed inset-0 overflow-y-auto`) AND its grid-view root div (a `overflow-x-hidden`-only div that was still confirmed via DevTools to be the actual vertical scroll container, a real CSS quirk), and `MarketsDashboard.tsx`'s root div (same `overflow-x-hidden`-only pattern). Each of the three pages had its own distinct scroll container, `<main>` alone did not cover all of them. Worth remembering: **check DevTools' Layout panel for which element gets the "scroll" badge, don't assume `<main>` is the only scroll container on a given page.**

**Morning Brief backend (Week 3, complete; frontend + cron route NOT yet built):**
- `src/lib/morning-brief-generation.ts` — `generateMorningBrief()`. Calls `getMarketsDashboard()` from `markets.ts` (reused, not duplicated — the model initially wrote a fresh snapshot-fetching implementation, was asked directly why it hadn't reused the existing pattern as instructed, and correctly found and switched to `getMarketsDashboard()` once questioned, a good example of the "ask directly rather than silently accept or silently override" working habit), derives top-3-gainers/top-3-losers via a pure `deriveMovers()` helper. Fetches last-24h `is_macro = true` news (max 8 articles). Fetches `calendar_events` in a 3-day forward window. Builds a single Gemini 3.6 Flash prompt instructing a 200-300 word desk-note narrative that explicitly only references calendar events if the list is genuinely non-empty (never fabricates a mention when nothing's confirmed, this mirrors the same honesty principle already established in the calendar's "Date TBD" UI treatment). Upserts on `brief_date` conflict key. Returns `{ success, briefDate, error? }`, empty optional data (no news/no calendar events) is explicitly not treated as an error, only a genuinely fatal step (the Gemini call itself, or the final upsert) produces a failure result.
- **Not yet built**: `/api/cron/morning-brief` route (should match `calendar-ingest`'s route shape, but run once daily not on any shorter cadence), the frontend page/component (mockup-first workflow required, get sign-off before writing the build prompt), cron-job.org registration.
...
- **Deferred: Economic Calendar's AI-estimated consensus layer.** `calendar_events` already has the columns (`estimated_consensus`, `estimate_rationale`, `estimate_generated_at`, `estimate_model_used`) but nothing populates them yet. The plan, decided but not built: since no free API provides genuine market-consensus forecast data (Trading Economics, Finnhub, FXStreet all ruled out, see Data Sources), a scheduled Gemini call would generate a plausible estimate for upcoming tracked releases, explicitly labeled in the UI as "Est." (distinct muted tag + tooltip, not styled as confirmed data) rather than presented as real consensus, this framing was deliberately chosen as a strength ("engineered around a real data-access constraint") not hidden as a weakness.
- **Deferred: Economic Calendar "Market Focus" panel.** Would show, on click/expand of an event, which assets are most exposed to it (e.g. "US CPI → USD, Treasuries, Gold"). Explicitly NOT built and not going to be built until a genuine cross-asset relevance mapping exists (a curated static table or lightweight scoring model), fabricating exposure data with no underlying logic was identified as a real credibility risk during design discussion and deliberately rejected, twice, after external design feedback suggested it. If this is ever built, it must be revealed on click, not a permanent always-visible panel (keeps the default calendar view's whitespace/restraint, matching the rest of the app's design instincts).
- **Deferred to Atlas V2, Week 5 (after the full 4-week V1 timeline, dashboard, landing page, docs, and GitHub launch)**: an in-app notification system. In-app only (bell icon + dropdown, no email/push, zero infra cost), quiet chime/badge, explicitly not jarring by design intent. Three trigger types: (1) a large yield/price move on any of the 16 watchlist assets past a threshold, (2) a previously-TBD Economic Calendar date becoming confirmed, (3) a user-favorited asset (favorite via right-click on a Markets Dashboard card, star icon, 1-3 assets max) getting a new News Engine mention. Requires a new `notifications` table, a simple favorites-flag table (no auth needed, single-user), and cron-based state-diffing to detect "this is new."
...
- **Economic Calendar uses FRED for release dates instead of any paid calendar API**, after live-testing and ruling out Trading Economics (no confirmed self-serve free tier), Finnhub's calendar endpoint (confirmed paid-only, live 403), and FXStreet (sales-gated). FRED is free and official but is fundamentally a historical-data database, not a forward-looking calendar product, meaning genuine gaps in upcoming dates (like the current CPI/NFP/GDP/PCE "Date TBD" state) are a real, honest limitation of the free-tier choice, not a bug. The UI's "Date pending" treatment (see Design Language) is the deliberate, honest response to this limitation rather than hiding or faking it.
- **Morning Brief reuses Gemini 3.6 Flash** (the same model/API already locked in for News Engine) rather than evaluating a separate model, since the free-tier constraint and quality bar are identical to the News Engine's already-resolved decision, re-running that comparison would have been redundant.
...
**Workflow decision worth carrying forward for any future significant UI/visual work**: for the News Engine redesign, an interactive HTML mockup was built first using Atlas's real design tokens (not generic/placeholder styling), iterated on directly with the user through targeted edits, and only turned into a detailed OpenCode build prompt (with the confirmed mockup HTML attached alongside the prompt text) once the user gave explicit sign-off. The user has explicitly asked for this mockup-first approach to be the default for future significant visual work, not just described in prose and built blind. This was reused successfully for Economic Calendar (4 mockup iterations, incorporating useful external design feedback while explicitly rejecting ideas that didn't fit real data constraints — see Known Gaps for the rejected "Market Focus" fabricated-data panel and weather-map metaphor styling) and should be reused again for Morning Brief's frontend.

**Economic Calendar's importance and status are two deliberately separate visual signals, don't conflate them.** The left accent bar communicates importance ONLY (opacity-scaled: high=100%, medium=45%, low=15%) using `--color-accent` (teal), while the right-side pill communicates status ONLY (Confirmed=teal-tinted, Date TBD=muted-grey with a hover tooltip). **Never use `--color-market-up`/`--color-market-down` (sage/dusty-coral) for importance or any other non-price-direction purpose** — these are reserved exclusively for actual price movement elsewhere in the app (Markets Dashboard, News Sentimeter), reusing them for Calendar importance was an early mistake this session (an AI builder defaulted to a reused token without realizing the reservation rule) and had to be corrected.

**A `.scrollbar-none` utility class exists in `globals.css`** (hides scrollbar chrome cross-browser: `scrollbar-width: none` for Firefox, `-ms-overflow-style: none` for legacy Edge/IE, `::-webkit-scrollbar { display: none }` for Chrome/Safari, while fully preserving scroll functionality) and should be applied to any new page-level scroll container going forward, matching the app's established "invisible scroll" feel. **A single page can have more than one real scroll container** (News Engine has two: its grid-view root and its article modal, discovered because an `overflow-x-hidden`-only div was, via a real CSS quirk, also functioning as the vertical scroll container). Always confirm via browser DevTools' Layout panel (look for the "scroll" badge on an element) rather than assuming `<main>` alone covers a given page.
...
25. **Economic Calendar data source decided**: Trading Economics, Finnhub's calendar endpoint, and FXStreet all live-tested and ruled out (no free self-serve tier or confirmed paid-only), FRED chosen as the only viable free option, with the honest limitation (gaps in upcoming dates) surfaced deliberately in the UI rather than hidden
26. **`calendar_events` schema built and executed**, including CHECK constraints on category/importance/status, a partial index for a future estimate-generation query pattern, RLS matching existing conventions
27. **Two real FRED API bugs found and fixed via live testing**: the plural `releases/dates` endpoint silently ignoring `release_id`, and a fixed ascending limit failing for high-frequency releases like FOMC (3748 total entries); both required actual live curl testing against the real API to discover, neither was documented anywhere obvious
28. **`TRACKED_RELEASES` curated allowlist built** (5 releases: CPI/NFP/GDP/PCE/FOMC), ISM/PMI confirmed genuinely absent from FRED's full release catalog (not just missed)
29. **Calendar ingestion, cron route, and live cron-job.org daily job built and verified end-to-end**, including a full debugging arc (bad data from the buggy endpoint cleared from Supabase, re-verified with correct data after each fix)
30. **`scripts/check-release-dates.ts` diagnostic tool built**, including fixing a real `.env.local`-vs-`.env` dotenv loading bug along the way
31. **Economic Calendar frontend built through 4 mockup iterations** (v1 through v4), incorporating useful external design critique while explicitly rejecting ideas that didn't fit real data (fabricated asset-exposure ratings, weather-map metaphor styling, country flags), signed off and implemented
32. **Three real post-build bugs found and fixed via user testing + browser inspection, not assumed from code**: a Sidebar white-box bug specific to the Calendar nav item, a layout bug forcing users to scroll past empty days to reach real content (fixed by filtering to only days with events, not just trailing empties), and app-wide scrollbar visibility across 3 pages (root-caused via actual DevTools inspection after a prose-based AI investigation attempt failed to find the real answer)
33. **Morning Brief schema and generation logic built and verified**: `morning_briefs` table live, `generateMorningBrief()` correctly reuses `getMarketsDashboard()` (after being directly questioned about not doing so initially), builds a Gemini prompt that never fabricates calendar mentions when nothing's confirmed, upserts on `brief_date`. Cron route, frontend, and cron-job.org registration are the explicit next step.

## Timeline Status

As of 2026-08-16: **Week 1 and Week 2 complete. Week 3's Economic Calendar module is fully complete and verified end-to-end in the browser** (schema, FRED integration with two real bugs found/fixed, curated allowlist, ingestion, live daily cron-job.org job, frontend through 4 signed-off mockup iterations, plus three post-build bugs found via real testing and fixed). **Morning Brief's backend (schema + generation logic) is built and verified; its cron route, frontend, and cron-job.org registration are the immediate next step**, not yet started. Roughly 1.5 weeks remain overall.
...
**Week 3: Economic Calendar DONE. Morning Brief backend DONE, frontend + cron next.**
- ~~Economic Calendar data source decision~~ Done: FRED chosen after live-ruling-out Trading Economics/Finnhub/FXStreet
- ~~`calendar_events` schema, FRED integration, curated allowlist, ingestion, cron route~~ Done, including two real FRED API bugs found and fixed via live testing (see Data Sources section)
- ~~Live cron-job.org daily registration for calendar ingestion~~ Done, failure notifications on
- ~~Economic Calendar frontend~~ Done: 4 mockup iterations, signed off, built, three post-build bugs found via real testing and fixed (Sidebar white-box, empty-days layout, app-wide scrollbar visibility)
- ~~`morning_briefs` schema~~ Done, live in Supabase and `supabase_schema.sql`
- ~~Morning Brief generation logic~~ Done: `generateMorningBrief()` in `morning-brief-generation.ts`, reuses `getMarketsDashboard()`, calls Gemini, upserts on `brief_date`
- **Not yet done, immediate next step**: Morning Brief cron route (`/api/cron/morning-brief`, daily cadence), frontend (mockup-first workflow, get sign-off before building), cron-job.org registration and verification
- Two design ideas from an externally-sourced design document (see Design Language section context) were parked here as relevant rather than built during Week 1: a Palantir-style causal-chain presentation ("Brent Crude ▲2.4% → CAUSES: ... → IMPACT: ...") and an "AI Observation" panel style with short analytical takes and confidence scores. Both map closely to what News Engine/Morning Brief are already trying to do, still not built, worth considering if time allows.
- Also parked from this week, not urgent: a Week 5/V2 in-app notifications system (fully scoped, see Known Gaps) and an Economic Calendar "Market Focus" click-reveal panel (needs real cross-asset data work first, see Known Gaps)
...
**Antigravity ("Anti") returned to use during Week 3** after its earlier quota exhaustion, and performed well on real investigation tasks (e.g. correctly root-causing a Sidebar focus-ring/border bug with a genuinely insightful explanation, not just a plausible guess). It also hit a fresh individual quota limit mid-session (stated refresh date should be checked fresh, not assumed current) and had a one-off stuck-installer issue (an update spun indefinitely; killing the process and relaunching, or checking for a stale lock file, resolved it). Model quota exhaustion mid-task should be expected as a recurring event across all these free-tier tools, not a one-time fluke, always confirm which model is actually live before trusting a session's output style/quality against prior expectations for that specific model.

**A genuinely important lesson from Week 3's scrollbar debugging**: a long, technically-worded AI investigation response is not automatically a verified answer. One attempt to root-cause the Markets Dashboard scrollbar bug produced several paragraphs of plausible-sounding reasoning, built two separate theories, explicitly abandoned both mid-response, and landed back on "probably a browser cache issue," the exact same non-answer offered before the investigation started, just with more words around it. The actual answer was found in under a minute by the user directly inspecting the page in browser DevTools (checking which specific element gets flagged with the "scroll" badge in the Layout panel). When a coding tool has real inspection capability available, prefer using it directly over another round of prose-based reasoning about code that hasn't been empirically checked.

**Verification effort should scale to the risk of the change, not be applied uniformly.** This was an explicit mid-session recalibration after the user pushed back on excessive ceremony for trivial edits: full `git diff` + `npx tsc --noEmit` + a careful read-through remains the standard for new logic, multi-file changes, or anything touching data/business logic (this caught several of the real bugs listed above). For small, single-line, low-risk mechanical edits (a class name added, a rename applied consistently across a few call sites), a quick `git status`/`git diff` glance is sufficient, don't demand the full formal cycle for changes that obviously can't hide much risk.

**When a model's own report contradicts what was explicitly asked** (e.g., told to reuse an existing function but visibly wrote a new one instead), the effective move proven this week is to ask it directly to explain the discrepancy, rather than either silently accepting the deviation or silently overriding it without discussion. This got a model to honestly find and switch to a genuinely better existing function (`getMarketsDashboard()`) after being questioned, producing a better outcome than either alternative would have.

**Be skeptical by default of AI-generated "here's a free API for that" suggestions**, especially from search-summary/AI-Mode-style tools rather than a direct, verifiable search result. Week 3 repeatedly encountered confidently-worded suggestions (Trading Economics, Finnhub's calendar endpoint specifically, Yahoo Finance undocumented endpoints, scraping Investing.com) that were wrong, unverifiable, or directly contradicted by this project's own live testing. Live-test any such claim before trusting it, regardless of how confident or detailed the suggestion sounds.
