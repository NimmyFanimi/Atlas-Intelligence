...
- `src/lib/data-sources/tracked-releases.ts` — `TRACKED_RELEASES`, the curated V1 allowlist: CPI (`10`), NFP (`50`), GDP (`53`), PCE (`54`), FOMC (`101`). Each entry has `primaryName` (short/acronym, e.g. "US NFP") and `secondaryName` (full name, e.g. "Employment Situation"), plus `category`/`importance`/`country`. ISM/PMI deliberately excluded, FRED simply does not carry it in its release catalog (confirmed by searching the full ~330-release list), this is a source limitation not a budget one.
- `src/lib/calendar-ingestion.ts` — `ingestUpcomingCalendarEvents()`, loops `TRACKED_RELEASES`, calls `fetchNextReleaseDate` per release (not a single batched call, this was the whole point of the endpoint-bug fix), upserts on `(fred_release_id, release_date)` with per-entry error isolation matching the style already established in `ingest-market-snapshots.ts`.
