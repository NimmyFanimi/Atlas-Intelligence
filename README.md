...
![Atlas Intelligence banner](.github/screenshots/banner.png)
...
- **News Engine:** AI-summarised financial news with asset tagging and sentiment scoring.
...
**Known, accepted limitation:** EIA's daily petroleum spot price table only updates weekly (Wednesdays). WTI and Brent prices shown can be up to ~8 days old at any given time. This is normal EIA publication behaviour, not an ingestion bug.
...
- **EIA data lag:** WTI/Brent prices can be up to ~8 days old (see Data Sourcing above). Confirmed as expected EIA behaviour via three independent sources, not a bug.
...
- **FOMC dates are sourced from an unofficial third-party mirror**, not FRED or the Federal Reserve directly, since no official free API for FOMC meeting dates exists. Clearly labelled in the UI.
...
2. **External Cron via cron-job.org and GitHub Actions:** Vercel's Hobby (free) tier only allows 2 total cron jobs and a once-daily minimum frequency. High-frequency ingestion routes (market snapshots, calendar) run on [cron-job.org](https://cron-job.org)'s free tier instead. Morning Brief, which only needs to run once daily, was migrated to Vercel's own native Cron Jobs after its Gemini generation time grew past cron-job.org's hard 30-second external timeout. News Engine's cron was later migrated a second time, off cron-job.org entirely and onto a GitHub Actions scheduled workflow, which removes the 30-second ceiling altogether (GitHub Actions runs are effectively unlimited for a public repo) and was the change that finally made a multi-day reliability debugging effort tractable, several of the fixes described in "How This Was Built" were only possible to verify in real time because a run could be triggered manually on demand rather than waiting for the next scheduled tick. **Implication:** live data freshness for the remaining cron-job.org routes still depends on that service's uptime as a real external dependency, while News Engine's freshness now depends on GitHub Actions' uptime instead.
3. **Separate Gemini API keys per feature, to stay inside a shared free-tier daily quota:** Morning Brief and News Engine both call Gemini 3.6 Flash, and initially shared one API key and its free-tier 20-requests-per-day cap. As News Engine's own call volume grew (multiple articles analysed per cron run, run every few hours), it alone came close to and then exceeded that shared cap, which also put Morning Brief's single daily call at risk of being crowded out. The fix was giving Morning Brief its own dedicated Gemini API key on a separate Google account, so the two features no longer draw from the same quota at all. News Engine also has its own secondary fallback key, on a third, separate account, used automatically if its primary key's daily quota is exhausted or a call times out.
