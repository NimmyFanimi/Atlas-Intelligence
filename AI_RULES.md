...
- `scripts/` — diagnostic/throwaway test scripts, run with `npx tsx scripts/<name>.ts` (e.g. `check-release-dates.ts` checks FRED `release/dates` per tracked release to confirm whether a date has been published yet).
