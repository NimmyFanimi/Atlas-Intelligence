...
async function main() {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${primaryKey}`;
  const body = JSON.stringify({
    contents: [{ parts: [{ text: 'Say hello in one word' }] }],
  });

  console.log('Firing two concurrent primary calls...');
